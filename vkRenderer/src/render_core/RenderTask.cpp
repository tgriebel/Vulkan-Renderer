...
void RenderTask::RenderViewSurfaces( CommandContext& cmdContext )
{
	...
	VkBuffer vertexBuffers[] = { view->vb->GetVkObject() };
	VkDeviceSize offsets[] = { 0 };
	vkCmdBindVertexBuffers( cmdContext.CommandBuffer(), 0, 1, vertexBuffers, offsets );
	vkCmdBindIndexBuffer( cmdContext.CommandBuffer(), view->ib->GetVkObject(), 0, VK_INDEX_TYPE_UINT32 );
...
{
	context.MarkerBeginRegion( view->GetName(), ColorToVector( Color::White ) );
